...
Petanque::Petanque(QSound *s,bool enable,QGraphicsItem *parent) : Defense(s,enable,3,0.5,15,0,1,15,40,80,20,8,QColor(188,195,197),parent)
{
this->desc = new QString;
this->degatZone= 10*pow(niveau,1.5);
this->updateDesc();
...
void Petanque::paint ( QPainter * painter, const QStyleOptionGraphicsItem * option, QWidget * widget)
{
    ...
    painter->setBrush(QPixmap(":/defenses/petanque.png").scaled(35,35));
...
QPainterPath Petanque::shape() const
 {
     QPainterPath path;
...
void Petanque::updateDesc()
{
    QString res ("Joueur de petanque niveau ");
    ...
    temp = QString("\nDegat de zone: ");
    res = res+temp;
    temp = QString(QString::number(this->degatZone));
    res = res+temp;
    if (this->niveau==0)
    {
    temp = QString("\nAmeliorer: ");
...
float Petanque::getDegatZone() const
{
